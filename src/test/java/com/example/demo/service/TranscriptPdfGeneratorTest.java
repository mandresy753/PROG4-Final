...
class TranscriptPdfGeneratorTest {
  ...
  void generate() {
    var student = User.builder()
        ...
        .lastName("Rakoto").firstName("Jean")
        .email("jean@test.com").role(UserRole.STUDENT).build();

    var courseLine = TranscriptCourseLine.builder()
        .courseRef("PROG1").courseTitle("Programmation 1")
        .creditCount(6).average(new BigDecimal("14")).validated(true)
        .status(TranscriptStatus.COMPLETE).build();

    var yearTranscript = YearTranscript.builder()
        .student(student)
        .academicYear(AcademicYear.builder().label("2025-2026").build())
        .courses(List.of(courseLine))
        .generalAverage(new BigDecimal("14"))
        .totalCredits(6).validatedCredits(6)
        .status(TranscriptStatus.COMPLETE).build();

    var fullTranscript = FullTranscript.builder()
        .student(student)
  ...
  void generate_multipleYears() {
    ...
    var courseLine1 = TranscriptCourseLine.builder()
        .courseRef("PROG1").courseTitle("Programmation 1")
        .creditCount(6).average(new BigDecimal("14")).validated(true)
        .status(TranscriptStatus.COMPLETE).build();
    var courseLine2 = TranscriptCourseLine.builder()
        .courseRef("WEB1").courseTitle("Web 1")
        .creditCount(6).average(new BigDecimal("12")).validated(true)
        .status(TranscriptStatus.COMPLETE).build();

    var year1 = YearTranscript.builder()
        .student(student)
        .academicYear(AcademicYear.builder().label("2024-2025").build())
        .courses(List.of(courseLine1))
        .generalAverage(new BigDecimal("14"))
        .totalCredits(30).validatedCredits(30)
        .status(TranscriptStatus.COMPLETE).build();

    var year2 = YearTranscript.builder()
        .student(student)
        .academicYear(AcademicYear.builder().label("2025-2026").build())
  ...
  void generate_withNullAverages() {
    ...
    var courseLine = TranscriptCourseLine.builder()
        .courseRef("PROG1").courseTitle("Programmation 1")
        .creditCount(6).average(null).validated(false)
    ...
    var yearTranscript = YearTranscript.builder()
        ...
        .courses(List.of(courseLine))
        .generalAverage(null)
        .totalCredits(6).validatedCredits(0)
        .status(TranscriptStatus.PROVISIONAL).build();
