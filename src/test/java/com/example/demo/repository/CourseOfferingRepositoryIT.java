...
class CourseOfferingRepositoryIT extends FacadeIT {
  ...
  void shouldFindByGroupIdAndAcademicYearId() {
    ...
    var group = groupRepository.save(
        JGroup.builder().reference("K1").track(Track.TRONC_COMMUN).build());

    var course = courseRepository.save(
        JCourse.builder().ref("PROG1").title("Programmation 1")
    ...
    var offering = courseOfferingRepository.save(
        JCourseOffering.builder()
            .course(course).academicYear(year)
            .groups(new HashSet<>(Set.of(group))).build());

    var result = courseOfferingRepository.findByGroupIdAndAcademicYear_Id(group.getId(), year.getId());
  ...
  void shouldFindByCourseIdAndAcademicYearIdAndGroupId() {
    ...
    var group = groupRepository.save(
        JGroup.builder().reference("K2").track(Track.TRONC_COMMUN).build());

    var course = courseRepository.save(
        JCourse.builder().ref("WEB1").title("Web 1")
            .creditCount(6).track(Track.TRONC_COMMUN).semester(Semester.S1).build());
    ...
    var result = courseOfferingRepository.findByCourse_IdAndAcademicYear_IdAndGroupId(
  ...
  void shouldFindByAcademicYearId() {
    ...
    var group = groupRepository.save(
        JGroup.builder().reference("K3").track(Track.TRONC_COMMUN).build());

    var course = courseRepository.save(
        JCourse.builder().ref("BASE1").title("Base de données")
            .creditCount(6).track(Track.TRONC_COMMUN).semester(Semester.S1).build());
